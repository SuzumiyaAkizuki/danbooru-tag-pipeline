...
def run(config):
        ...
        try:
            ...
            for entry in data:
                ...
                if entry_time <= last_update_time:
                    ...
                    reached_end = True
                    break
                if isinstance(entry.get('other_names'), list):
                    entry['other_names'] = str(entry['other_names'])
                new_records_batch.append(entry)
            ...
            if not reached_end:
                ...
                if current_page > 900:
                    ...
                    if new_records_batch:
                        ...
                        df_temp.to_csv(temp_csv_file, mode='a', header=not temp_csv_file.exists(), index=False,
                                       encoding='utf_8_sig')
                ...
                if (current_page - 1) % 20 == 0:
                    ...
                    with open(progress_file, 'w') as f:
                        ...
                        if current_upper_bound: f.write(f"{current_upper_bound}\n")
    ...
    if new_records_batch:
        df_temp = pd.DataFrame(new_records_batch)
        df_temp.to_csv(temp_csv_file, mode='a', header=not temp_csv_file.exists(), index=False, encoding='utf_8_sig')
    ...
    if temp_csv_file.exists():
        ...
        click.secho(f"[Fetch Wiki] Wiki 数据合并成功！保存为 {parquet_path.name} (总行数: {len(df_final)})", fg="green")
